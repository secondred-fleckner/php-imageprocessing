...
//              maurice@svay.Com
    ...
    class FaceDetector {
        ...
        public function __construct($detection_file = null) {
            
            $this->faces = array();
            
            if ($detection_file == null)
            {
                $detection_file = __DIR__.'/FaceDetection.dat';
            }
        ...
        public function face_detect($file) {
            ...
            else if (is_file($file) || strpos($file, 'http') !== false) {
        ...
        public function getFaceImageSource($floatPadding = 1.5, $bAsResource=false) {
             
            $arrFace = $this->face;
            $arrFace['x'] -= 0.5*($arrFace['w']*$floatPadding - $arrFace['w']);
            if ($arrFace['x'] < 0)
            {
                $arrFace['w'] += $arrFace['x'];
                $arrFace['x'] = 0;
            }
            $arrFace['y'] -= 0.5*($arrFace['w']*$floatPadding - $arrFace['w']);
            if ($arrFace['y'] < 0)
            {
                $arrFace['y'] = 0;
            }
            
            $arrFace['w'] *= $floatPadding;
                            
            if ($arrFace['w'] > 0)
            {
                $im = imagecreatetruecolor($arrFace['w'], $arrFace['w']);
                imagecopyresampled($im, $this->canvas, 0, 0, $arrFace['x'], $arrFace['y'], $arrFace['w'], $arrFace['w'], $arrFace['w'], $arrFace['w']);    
            }
